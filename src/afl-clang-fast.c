...
static void find_obj(u8 *argv0) {
  ...
  FATAL("Unable to find 'afl-llvm-rt.o' or 'afl-llvm-pass.so'. Please set "
        "AFL_PATH");
}
...
static void edit_params(u32 argc, char **argv) {
  ...
  u8 *name;
#ifdef AFL_CLANG_NSFUZZ
  u8 compiling_asm = 0;
#endif /* ifdef AFL_CLANG_NSFUZZ */
...
#ifdef AFL_CLANG_NSFUZZ
  if (getenv("NSFUZZ_PARSE_ANNOTATION")) {
    cc_params[cc_par_cnt++] = "-Xclang";
    cc_params[cc_par_cnt++] = "-load";
    cc_params[cc_par_cnt++] = "-Xclang";
    cc_params[cc_par_cnt++] =
        alloc_printf("%s/nsfuzz-annotation-pass.so", obj_path);
    cc_params[cc_par_cnt++] = "-D_NSFUZZ_PARSING";
...
#ifdef AFL_CLANG_INSTRUMENT
  cc_params[cc_par_cnt++] = "-Xclang";
  cc_params[cc_par_cnt++] = "-load";
  cc_params[cc_par_cnt++] = "-Xclang";
  cc_params[cc_par_cnt++] =
      alloc_printf("%s/afl-definition-checker.so", obj_path);
#endif
#endif /* ^USE_TRACE_PC */
...
#ifdef AFL_CLANG_NSFUZZ
    if (strlen(cur) > 2 && cur[strlen(cur) - 1] == 'S' &&
        cur[strlen(cur) - 2] == ".") {
...
#ifdef AFL_CLANG_NSFUZZ
  if (compiling_asm) {
    cc_params[cc_par_cnt++] = "-D_NO_NSFUZZ_IN_ASM";
  }
  cc_params[cc_par_cnt++] =
      alloc_printf("-include%s/llvm_mode/afl-nsfuzz-rt.h", obj_path);
#endif
  ...
  if (maybe_linking) {
    ...
    case 0:
      ...
      cc_params[cc_par_cnt++] =
          alloc_printf("%s/afl-definition-checker.o", obj_path);
      break;
...
#ifdef AFL_CLANG_STATEAFL
  if (!strcmp(name, "afl-clang-fast++")) {
    cc_params[cc_par_cnt++] =
        alloc_printf("%s/afl-llvm-rt-state-tracer.o.cpp.o", obj_path);
  } else {
    cc_params[cc_par_cnt++] =
