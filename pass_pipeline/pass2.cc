...
static void register_pass2(const PassManagerBuilder &,
...
bool pass2::runOnModule(Module &M) {
  ...
  FunctionType *pass2function_t = FunctionType::get(i64_type, {}, false);

  FunctionCallee pass2function =
      M.getOrInsertFunction("function2", pass2function_t);
    ...
    for (BasicBlock &basic_block : function) {
      for (Instruction &instruction : make_early_inc_range(basic_block)) {
        if (auto *call_instruction = dyn_cast<CallInst>(&instruction)) {
          ...
          if (callee_name == "function1") {
            IRBuilder<> ir_builder(call_instruction);
            ir_builder.CreateCall(pass2function, {});
          }
        }
      }
    }
